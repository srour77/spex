...
class VendorController {
  ...
  resetPassword: RequestHandler<any, APIResponse, { password: string }, { token: string }> = async(req, res, next) => {
    jwt.verify(req.query.token, String(process.env.ResetPasswordSecret), async (err, decoded) => {
      ...
      if(decoded.role != Roles.vendor || await this.db.getVendorCount(decoded.email) == 0) { res.status(StatusCodes.BAD_REQUEST).json({ message: 'invalid email', success: true}); return; }
  ...
  getOders: RequestHandler<any, APIResponse & { orders: Array<Pick<Order, 'id'> & { customer: Pick<Customer, 'name' | 'email'> } & { products: Array<Pick<Product_Order, 'price' | 'itemNo'> & { product: Pick<Product, 'id' | 'name' | 'desc'>  }> }> }> = async(req, res, next) => {
    const { [Roles.vendor]: { id } } = res.locals;
